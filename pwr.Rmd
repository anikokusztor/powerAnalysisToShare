...
### Explanation
This code mirrors your initial structure but is adapted for a notebook format:

1. **Parameters**: Set the number of simulations, sample sizes, and `CDSslope`.
2. **Function Definition**: `simData` simulates data for each sample size, retaining your exact logic.
3. **Simulation Loop**: For each sample size, it simulates data, fits the model, and extracts p-values.
4. **Power Calculation and Plot**: Computes the proportion of significant p-values and visualizes the power curve.
...
Next we are running simulations. 
...
# Access parameters using params$param_name
sample_sizes <- params$sample_size
CDSslope <- params$effect_size
nsim <- params$nsim
...
for (iii in sample_sizes){
  ...
  for (ii in 1:nsim){
    ...
    cat('Starting simulation', sim, 'of', nsim, 'at', tstartfull_format, '\n')
